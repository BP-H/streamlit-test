...
def render_agent_insights_tab() -> None:
    ...
    with st.container():
        ...
        with st.expander("📘 Notes", expanded=False):
            note_col, rfc_col = st.columns(2)
            diary_note = note_col.text_input("Add note")
            rfc_input = rfc_col.text_input("Referenced RFC IDs (comma separated)")
            ...
            export_md_col, export_json_col = st.columns(2)
            with export_md_col:
                st.download_button(
        ...
        with st.expander("Proposed RFCs", expanded=False):
            rfc_dir = Path('rfcs')
            filter_col, preview_col = st.columns(2)
            filter_text = filter_col.text_input("Filter RFCs")
            preview_all = preview_col.checkbox("Preview full text")
