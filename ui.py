    ...
    try:
        ...
        with left_col:
            ...
            with st.expander("Developer Tools"):
                ...
                with dev_tabs[0]:
                    if "cosmic_nexus" in globals() and DATABASE_AVAILABLE:
                        user = safe_get_user()
                ...
                with dev_tabs[2]:
                    ...
                    if st.button("Run Audit") and hid:
                        if "dispatch_route" in globals() and DATABASE_AVAILABLE:
                            try:
                                with SessionLocal() as db:
                        ...
                        elif "run_full_audit" in globals() and DATABASE_AVAILABLE:
                            try:
                                with SessionLocal() as db:
                                    with st.spinner("Working on it..."):
...
def ensure_database_exists() -> bool:
    """Create harmonizers table if it doesn't exist."""
    try:
        secrets = get_st_secrets()
        ...
        logger.warning("Unexpected DB init failure: %s", exc)
        return False
